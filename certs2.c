...
static pthread_once_t gctx_once = PTHREAD_ONCE_INIT;



// OpenSSL 3 provider initialization
static void openssl3_init(void) __attribute__((constructor));
static void openssl3_init(void)
{
    if (!OPENSSL_init_ssl(OPENSSL_INIT_LOAD_CONFIG, NULL)) {
        log_msg(LGG_ERR, "OPENSSL_init_ssl failed");
        exit(1);
    }
    if (!OSSL_PROVIDER_load(NULL, "default")) {
        log_msg(LGG_ERR, "Could not load default provider");
        exit(1);
    }
}
...
#define SAN_STR_SIZE PIXELSERV_MAX_SERVER_NAME + 4 /* max("IP:", "DNS:") = 4 */
    ...
    // ZufÃ¤lliges NotBefore-Datum zwischen -864000 und -172800 Sekunden
...
{
    pthread_once(&gctx_once, init_global_ctx);
    return g_sslctx;
}
...
// Kompatible No-op Implementierungen fÃ¼r OpenSSL 1.1+/3.x
