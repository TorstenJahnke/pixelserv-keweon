...
/* Add prototype for generate_cert so that no implicit int is assumed */
...
inline int sslctx_tbl_get_sess_purge() {
  return SSL_CTX_sess_cache_full(g_sslctx);
}

static int sslctx_tbl_insert(const char *cert_name, SSL_CTX *sslctx,
                             int ins_idx);
static SSL_CTX *create_child_sslctx(const char *full_pem_path,
                                    const STACK_OF(X509_INFO) * cachain);
...
void conn_stor_flush() {
  if (conn_stor_max < 0 || conn_stor_last < 0 ||
      conn_stor_last <= conn_stor_max / 2)
    return;
  int threshold = conn_stor_max / 2;
  pthread_mutex_lock(&cslock);
  for (; conn_stor_last >= threshold && conn_stor[conn_stor_last] != NULL;
       conn_stor_last--) {
...
void sslctx_tbl_init(int tbl_size) {
  ...
  if (!sslctx_tbl) {
    ...
    sslctx_tbl_cnt_hit = sslctx_tbl_cnt_miss = sslctx_tbl_cnt_purge =
...
static int cmp_sslctx_reuse_count(const void *p1, const void *p2) {
  /* reverse order */
  return ((sslctx_cache_struct *)p2)->reuse_count -
         ((sslctx_cache_struct *)p1)->reuse_count;
}

static int cmp_sslctx_certname(const void *k, const void *p) {
  return strcmp(((sslctx_cache_struct *)k)->cert_name,
                ((sslctx_cache_struct *)p)->cert_name);
}

void sslctx_tbl_load(const char *pem_dir, const STACK_OF(X509_INFO) * cachain) {
  FILE *fp;
  char *fname = NULL, *line;
  if ((line = malloc(PIXELSERV_MAX_PATH)) == NULL ||
      (fname = malloc(PIXELSERV_MAX_PATH)) == NULL) {
  ...
  qsort(SSLCTX_TBL_ptr(0), sslctx_tbl_end, sizeof(sslctx_cache_struct),
        cmp_sslctx_certname);
quit_load:
  free(fname);
  free(line);
...
#ifdef DEBUG
  printf("%s: now %d last_flush %d", __FUNCTION__, pixel_now,
         sslctx_tbl_last_flush);
#endif
...
static int sslctx_tbl_lookup(char *cert_name, int *found_idx, int *ins_idx) {
  ...
  *ins_idx = -1;
  if (!cert_name || !found_idx || !ins_idx) {
    log_msg(LOG_ERR,
            "Invalid params. cert_name: %s. found_idx: %d, ins_idx: %d",
  ...
  found = bsearch(&key, SSLCTX_TBL_ptr(0), sslctx_tbl_end,
                  sizeof(sslctx_cache_struct), cmp_sslctx_certname);
...
static int sslctx_tbl_insert(const char *cert_name, SSL_CTX *sslctx,
                             int ins_idx) {
  if (cert_name == NULL || sslctx == NULL || ins_idx >= sslctx_tbl_size ||
      ins_idx < 0) {
...
static int sslctx_tbl_cache(const char *cert_name, SSL_CTX *sslctx,
                            int ins_idx) {
  int ret = -1;
...
void ssl_init_locks() {
  ...
  CRYPTO_set_locking_callback(
      (void (*)(int, int, const char *, int))ssl_lock_cb);
#endif
}
...
#define SAN_STR_SIZE PIXELSERV_MAX_SERVER_NAME + 4 /* max("IP:", "DNS:") = 4 \
  ...
  X509_gmtime_adj(X509_get_notAfter(x509),
                  3600 * 24 * 390L);  // cert valid for 390 days
  X509_set_issuer_name(x509, issuer);
  X509_NAME *name = X509_get_subject_name(x509);
  X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, (unsigned char *)pem_fn,
                             -1, -1, 0);
  ...
  tld_tmp = (dot_count == 3 &&
             (atoi(tld) > 0 || (atoi(tld) == 0 && strlen(tld) == 1)))
                ? "IP"
                : "DNS";
  snprintf(san_str, SAN_STR_SIZE, "%s:%s", tld_tmp, pem_fn);
  if ((ext = X509V3_EXT_conf_nid(NULL, NULL, NID_subject_alt_name, san_str)) ==
      NULL)
    goto free_all;
  X509_add_ext(x509, ext, -1);
  X509_EXTENSION_free(ext);
  if ((ext = X509V3_EXT_conf_nid(NULL, NULL, NID_ext_key_usage,
                                 "TLS Web Server Authentication")) == NULL)
    goto free_all;
  X509_add_ext(x509, ext, -1);
  ...
  if (fp == NULL) {
    log_msg(LGG_ERR, "%s: failed to open file for write: %s", __FUNCTION__,
            fname);
    goto free_all;
  }
  // Server-Zertifikat
...
void cert_tlstor_init(const char *pem_dir, cert_tlstor_t *ct) {
  ...
  fp = fopen(cert_file, "r");
  if (!fp ||
      !PEM_read_PrivateKey(fp, &ct->privkey, pem_passwd_cb, (void *)pem_dir))
    log_msg(LGG_ERR, "%s: failed to load ca.key", __FUNCTION__);
  fclose(fp);

  /* --- START Workers Pool for cert generation ------------------------- */
  for (int i = 0; i < 4; i++) {
    pthread_t tid;
...
void *cert_generator(void *ptr) {
  ...
  for (;;) {
    int ret;
    if (fd == -1)
      log_msg(LGG_ERR, "%s: failed to open %s: %s", PIXEL_CERT_PIPE,
              strerror(errno));
    ...
    ssize_t cnt;
    if ((cnt = read(fd, buf + strlen(half_token),
                    PIXELSERV_MAX_SERVER_NAME * 4 - strlen(half_token))) == 0) {
#ifdef DEBUG
      printf("%s: pipe EOF\n", __FUNCTION__);
#endif
      close(fd);
      fd =
          open(PIXEL_CERT_PIPE, O_RDONLY | O_NONBLOCK); /* non block required */
      continue;
    }
    if (!cnt) continue;
    if ((size_t)cnt < PIXELSERV_MAX_SERVER_NAME * 4 - strlen(half_token)) {
      ...
      size_t i = 0;
      for (i = 1;
           buf[PIXELSERV_MAX_SERVER_NAME * 4 - i] != ':' && i < strlen(buf);
           i++)
        ;
    ...
    while (p_buf != NULL) {
      char cert_file[PIXELSERV_MAX_PATH];
      struct stat st;
      snprintf(cert_file, PIXELSERV_MAX_PATH, "%s/%s",
               ((cert_tlstor_t *)ct)->pem_dir, p_buf);
...
#define CB_ERR SSL_TLSEXT_ERR_ALERT_FATAL
  ...
  if (dot_count <= 1 || (dot_count == 2 && strlen(tld) == 2) ||
  ...
  if (stat(full_pem_path, &st) != 0) {
    ...
    /* dealy if cert is not existing */
    {
      struct timespec delay = {0, 150 * 1000000}; /* 150ms */
  ...
  submit_missing_cert:
    /* Job enqueuen instead Pipe-Write */
    {
  ...
  if (NULL == (sslctx = create_child_sslctx(full_pem_path, cbarg->cachain))) {
    log_msg(LGG_ERR, "%s: fail to create sslctx or cache %s", __FUNCTION__,
            pem_file);
    cbarg->status = SSL_ERR;
    rv = CB_ERR;
    goto quit_cb;
...
static SSL_SESSION *get_session(SSL *ssl, unsigned char *id, int idlen, int
*do_copy) { return NULL;
}
*/
...
static SSL_CTX *create_child_sslctx(const char *full_pem_path,
  ...
  SSL_CTX_set_options(sslctx, SSL_OP_SINGLE_DH_USE | SSL_MODE_RELEASE_BUFFERS |
  ...
  SSL_CTX_set_session_cache_mode(
      sslctx, SSL_SESS_CACHE_NO_AUTO_CLEAR | SSL_SESS_CACHE_SERVER);
  ...
  if (SSL_CTX_use_certificate_file(sslctx, full_pem_path, SSL_FILETYPE_PEM) <=
          0 ||
      SSL_CTX_use_PrivateKey_file(sslctx, full_pem_path, SSL_FILETYPE_PEM) <=
          0) {
    SSL_CTX_free(sslctx);
    ...
    for (i = sk_X509_INFO_num(cachain) - 1; i >= 0; i--) {
      if ((inf = sk_X509_INFO_value(cachain, i)) && inf->x509 &&
        ...
        log_msg(LGG_ERR, "%s: cannot add CA cert %d\n", i,
                __FUNCTION__); /* X509_ref_up requires >= v1.1 */
        return NULL;
      }
    }
  }
...
SSL_CTX *create_default_sslctx(const char *pem_dir) {
  ...
  SSL_CTX_set_options(g_sslctx, SSL_MODE_RELEASE_BUFFERS |
                                    SSL_OP_NO_COMPRESSION | SSL_OP_NO_SSLv2 |
...
int is_ssl_conn(int fd, char *srv_ip, int srv_ip_len, const int *ssl_ports,
                int num_ssl_ports) {
  ...
  if (getnameinfo((struct sockaddr *)&sin_addr, sin_addr_len, server_ip,
                  sizeof server_ip, port, sizeof port,
